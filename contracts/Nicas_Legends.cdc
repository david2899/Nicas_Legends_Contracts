...
pub contract Nicas_legends: NonFungibleToken {
    ...
    */
    ...
    pub resource interface CollectionPublic {
        ...
        pub fun borrowExperiaNFT(id: UInt64): &Nicas_legends.NFT? {
            post {
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            let token <- token as! @Nicas_legends.NFT
        ...
        pub fun borrowExperiaNFT(id: UInt64): &Nicas_legends.NFT? {
             let ref = &self.ownedNFTs[id] as auth &NonFungibleToken.NFT?
                return ref as! &Nicas_legends.NFT?
        }
    ...
    pub resource interface NFTCollectionReceiver {
      pub fun generateNFT(amount: UInt64, collection: Capability<&Nicas_legends.Collection{NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic}>)
    ...
    pub resource NFTTemplate: NFTCollectionReceiver {
        ...
        init(name: String,  metadata: {String: AnyStruct}, thumbnail: String, description: String, amountToCreate: UInt64,
         maximum: UInt64, collection: Capability<&Nicas_legends.Collection{NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic}>) { 
            self.metadata = metadata
            self.name = name
        ...
        pub fun generateNFT(amount: UInt64, collection: Capability<&Nicas_legends.Collection{NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic}>) {
            ...
            while i < Int(amount) {
             Nicas_legends.totalSupply = Nicas_legends.totalSupply + 1 
                let newNFT <- create NFT(id: Nicas_legends.totalSupply, name: self.name, metadata: self.metadata, thumbnail: self.thumbnail, description: self.description)
                collectionBorrow.deposit(token: <- newNFT)
                self.collectionNFT.append(Nicas_legends.totalSupply)
                self.counteriDs.append(Nicas_legends.totalSupply)
                i = i + 1
            }
    ...
    pub fun createEmptyCollectionNFT(name: String, metadata: {String:AnyStruct}): @NonFungibleToken.Collection {
        var newID = Nicas_legends.totalCollection + 1
        Nicas_legends.totalCollection = newID
        emit NewCollection(collectionName: name, collectionID: Nicas_legends.totalCollection)
